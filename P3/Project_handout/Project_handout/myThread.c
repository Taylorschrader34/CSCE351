...
void signalHandler( int signal )
{
	 ...
	 */
	 ...
	 if(signal == SIGALRM){
	 		for(k;k<THREADS;k++){
				//printf("status current before: %d\n",status[currentThread]);
				int tempThread = currentThread;
				currentThread = k;
				status[currentThread] = 2;
				//printf("status (new)current now: %d\n",status[currentThread]);
				status[tempThread] = 1;
				swapcontext(&context[tempThread],&context[currentThread]);
				//printf("status (new)current after: %d\n",status[currentThread]);
...
void cleanup() {
	...
	//printf("total Threads before: %d\n",totalThreads);
		if(status[currentThread] != 0){
			status[currentThread] = 0;
			totalThreads--;
			//printf("total Threads after: %d\n",totalThreads);
			//printf(a);
		}
	 }
